...
def proxy(proxy_address: tuple[str, int], server_adress: tuple[str, int]) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as proxy_socket:
        ...
        # * Fill in start (1)
        # Assigns an IP address and a port number to the proxy_socket (default port: 9998).
        proxy_socket.bind(proxy_address)
        print("Socket bind complete")
        # Enable proxy server's created socket to accept client connections.
        proxy_socket.listen()
        # * Fill in end (1)
        ...
        while True:
            try:
                ...
                # Accept function returns a 2-tuple: the first value is the client's socket with which to connect,
                # and the second value is the client's IP address and port to be used
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int], server_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            # Receive all data from the socket created for the client
            # Allow up to 8180 bytes of data to pass through socket
            ...
            try:
                ...
                # * Fill in start (4)
                # Send the response (already encoded) to the client through the client socket
